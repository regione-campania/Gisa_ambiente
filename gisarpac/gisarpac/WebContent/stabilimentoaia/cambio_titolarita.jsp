...
<%@page import="org.aspcfs.modules.aia.base.ImpresaAIA"%>
    <%@page import="org.aspcfs.modules.aia.base.Indirizzo"%>
...
<body>
<form name="CambioTitolarita" action="StabilimentoAIA.do?command=Cambio" onSubmit="return validateForm()" method="post">
...
function selezionaImpresa(impresaScelta){
	...
	document.getElementById('dati_impresa_id').readOnly = true;		
			...
			<tr id="tr_ragione_sociale_impresa" title="" size="70" required="true">
				...
				<td>
					<input type="text" id="ragione_sociale_impresa" name="ragione_sociale" disabled value="<%=StabilimentoDettaglio.getRagioneSociale()%>" size="70" required="true" title="inserire ragione sociale impresa">
			</td></tr>
			<tr id="tr_partita_iva_impresa" title="" maxlength="11" pattern="[0-9]{11}" required="true">
				<td class="formLabel">PARTITA IVA</td>
				<td>
					<input type="text" id="partita_iva_impresa" name="partita_iva" value="<%=StabilimentoDettaglio.getImpresa().getPartitaIva()%>" maxlength="11" pattern="[0-9]{11}" required="true" title="inserire 11 caratteri numerici"  onblur="myfunction();
var objImpresa;
function myfunction()
{
...
function selezionaImpresa(impresaScelta){
	...
	document.getElementById('partita_iva_impresa').readOnly = true;		
			...
			<tr id="tr_email_impresa" title="" size="50">
				...
				<td>
					<input type="text" id="email_impresa" disabled name="email_impresa" value="<%=StabilimentoDettaglio.getImpresa().getDomicilioDigitale()%>" size="50">
			</td></tr>
			...
			<tr id="tr_nome_rapp_legale" title="" required="true" pattern="[A-Za-z รง']{1,70}">
				...
				<td>
					<input type="text" id="nome_rapp_legale" name="nome_rapp_leg" value="<%=StabilimentoDettaglio.getImpresa().getSoggettoFisico().getNome()%>"  required="true" pattern="[A-Za-z รง']{1,70}" title="inserire nome rappresentante legale" onchange="document.getElementById('cf_rapp_legale').value = '';">
			</td></tr>
			<tr id="tr_cognome_rapp_legale" title="" required="true" pattern="[A-Za-z ']{1,70}">
				<td class="formLabel">COGNOME</td>
				<td>
					<input type="text" id="cognome_rapp_legale" name="cognome_rapp_leg" value="<%=StabilimentoDettaglio.getImpresa().getSoggettoFisico().getCognome()%>"  required="true" pattern="[A-Za-z ']{1,70}" title="inserire cognome rappresentante legale" onchange="document.getElementById('cf_rapp_legale').value = '';">
			</td></tr>
 				<tr id="tr_sesso_rapp_legale">
        			...
        			<td>
            			<select name="sesso_rapp_leg" id="sesso_rapp_legale"  onchange="document.getElementById('cf_rapp_legale').value = '';">
            					...
            					<%if(StabilimentoDettaglio.getImpresa().getSoggettoFisico().getSesso().equalsIgnoreCase("M")){ %>
            			        <option value="F">F</option>                 
            					<%}else if(StabilimentoDettaglio.getImpresa().getSoggettoFisico().getSesso().equalsIgnoreCase("F")){ %>
			...
			<tr id="tr_comune_nascita_estero_rapp_legale" title="" style="display: none;">
				...
				<td>
					<input type="text"  id="comune_nascita_estero_rapp_legale" name="comune_nascita_rapp_leg" value="<%=StabilimentoDettaglio.getImpresa().getSoggettoFisico().getComuneNascita()%>" style="display: none;">
			</td></tr>
 			<tr>
        		<td class="formLabel">DATA NASCITA</td>
        		<td>
        		
                	<input  placeholder="Inserisci data" type="text" id="data_nascita_rapp_legale" name="data_nascita_rapp_legale" autocomplete="off" value="<%=StabilimentoDettaglio.getImpresa().getSoggettoFisico().getDataNascitaString()%>">                
                	<script>
		...
		return false;
			...
			<tr id="tr_comune_residenza_estero_rapp_legale" title="" style="display: none;">
				...
				<td>
								...
								<input type="text" id="cap_residenza_rapp_legale" name="cap_soggfis"  required placeholder="CAP" value="<%=StabilimentoDettaglio.getImpresa().getSoggettoFisico().getIndirizzo().getCap()%>" size="5" maxlength="5" readonly="">
			...
			<tr id="tr_nome_resp_stab" title="">
				<td class="formLabel">NOME</td>
				<td>
					<input type="text"  disabled id="nome_resp_stab" name="nome_resp_stab" value="<%=StabilimentoDettaglio.getResponsabile().getNome()%>">
			</td></tr>
			<tr id="tr_cognome_resp_stab" title="">
				<td class="formLabel">COGNOME</td>
				<td>
					<input type="text" disabled id="cognome_resp_stab" name="cognome_resp_stab" value="<%=StabilimentoDettaglio.getResponsabile().getCognome()%>">
			</td></tr>
			<tr id="tr_cf_resp_stab" title="">
				<td class="formLabel">CODICE FISCALE</td>
				<td>
					<input type="text" disabled id="cf_resp_stab" name="cf_resp_stab" value="<%=StabilimentoDettaglio.getResponsabile().getCodFiscale()%>">
			</td></tr>
			<tr id="tr_sede_legale_id">
 				...
 				<tr id="tr_nazione_sede_legale">
        			...
        			<%=NazioniList.getHtmlSelect("nazione_sede_legale",StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getNazione())%>            			
        			</td>
 				</tr>
				...
				<td>
								...
								<input type="text" id="toponimo_sede_legale" name="toponimo_sede_leg" placeholder="TOPONIMO" value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getDescrizioneToponimo()%>" size="10" readonly="">
								<input type="text" id="via_sede_legale" name="via_sede_legale" placeholder="VIA" value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getVia()%>" size="38" readonly="">
								<input type="text" id="civico_sede_legale" name="civico_sede_legale" placeholder="CIVICO" value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getCivico()%>" size="10" readonly="">
								<input type="text" id="cap_sede_legale" name="cap_leg" placeholder="CAP" value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getCap()%>" size="5" maxlength="5" onkeydown="return false;" required="" autocomplete="off">
								<input type="text" id="comune_sede_legale" name="comune_sede_legale" placeholder="COMUNE" value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getDescrizioneComune()%>" size="30" readonly="">
								<input type="text" id="provincia_sede_legale" name="provincia_sede_legale" placeholder="PROVINCIA" value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getDescrizione_provincia()%>" size="18" readonly="">
					 			<input type="button" id="ins_indirizzo_sede_legale" value="INSERISCI INDIRIZZO" onclick="openCapWidget('toponimo_sede_legale','topIdSedeLegale','via_sede_legale','civico_sede_legale','comune_sede_legale','comuneIdSedeLegale','cap_sede_legale','provincia_sede_legale','provinciaIdSedeLegale')">     
			...
			<tr id="tr_comune_estero_sede_legale"  style="display: none;">
				<td class="formLabel">COMUNE ESTERO SEDE LEGALE</td>
					<td><input type="text" id="comune_estero_sede_legale" name="comune_estero_sede_legale" value="" style="display: none;"></td>
 	 				...
 	 				<input type="hidden" id="provinciaIdSedeLegale" name="cod_provincia_sede_legale" value=<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getIdProvincia()%>>	
 	 				<input type="hidden" id="comuneIdSedeLegale" name="cod_comune_sede_legale" value=<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getComune()%>>	
 	 				<input type="hidden" id="topIdSedeLegale" name="toponimo_sede_legale" value=<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getToponimo()%>>	
				<tr id="coordinate_sede_legale" style="display: none;">
				<td class="formLabel">COORDINATE</td>	
				<td>
								<input type="text" id="lat_sede_legale" name="latitudine_leg" placeholder="LATITUDINE" value="" onfocusout="this.value=this.value.replace(/[^0-9.]+/,''); if (this.value.length==0 || !this.value.includes('.') || this.value.charAt(0) == '.' || this.value.charAt(this.value.length-1) == '.') {alert('Valorizzare le coordinate nel formato xx.yyyy'); this.value=''; return false}" autocomplete="off">
								<input type="text" id="long_sede_legale" name="longitudine_leg" placeholder="LONGITUDINE" value="" onfocusout="this.value=this.value.replace(/[^0-9.]+/,''); if (this.value.length==0 || !this.value.includes('.') || this.value.charAt(0) == '.' || this.value.charAt(this.value.length-1) == '.') {alert('Valorizzare le coordinate nel formato xx.yyyy'); this.value=''; return false}" autocomplete="off">
...
if(document.getElementById("nazione_sede_legale").value!=106){
	nazioneResetNazi();
	document.getElementById("comune_estero_sede_legale").value="<%=StabilimentoDettaglio.getImpresa().getSedeLegaleObj().getComuneTesto()%>"
}

...
<table class="trails">
		<tr>
			<td><div align="left"><a href="Dpat.do">DPAT</a> &gt <a href="Dpat.do?command=Home&combo_area=<%=(dpat.getElencoStrutture().size()>1) ? "-1" :dpat.getElencoStrutture().get(0).getId()  %>&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>">Allegati DPAT</a> &gt <a href="DpatSDC.do?command=Details&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>">All. 5 Strumento di Calcolo <%=dpat.getAnno()%></a> &gt Attivita'</div></td>	
		</tr>
</table>
<input type="button" id="fine" name="fine" value="FINE LAVORO" onclick="window.location='Dpat.do?command=DpatDetailGenerale&id=<%=dpat.getId()%>&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>'"></input>
<br>

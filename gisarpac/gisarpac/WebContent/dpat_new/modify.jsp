...
<table class="trails">
		<tr>
			<td><div align="left"><a href="Dpat.do">DPAT</a> &gt <a href="Dpat.do?command=Home&combo_area=<%=(dpat.getElencoStrutture().size()>1) ? "-1" :dpat.getElencoStrutture().get(0).getId()  %>&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>">Allegati DPAT</a> &gt <a href="DpatSDC.do?command=AddModify&combo_area=<%=(dpat.getElencoStrutture().size()>1) ? "-1" :dpat.getElencoStrutture().get(0).getId()  %>&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>">All. 5 Strumento di Calcolo <%=dpat.getAnno()%></a> &gt Attivita' ASL <%=ListaAsl.getSelectedValue(dpat.getIdAsl()) %> </div></td>	
		</tr>
</table>
...
<%  if(dpat.isCompleto()==false){
	...
	<font color=red>SI RICORDA CHE PER CONSENTIRE IL SALVATAGGIO AUTOMATICO DEL DOCUMENTO BISOGNA CLICCARE AL DI FUORI DELLA CASELLA DOPO AVER INSERITO I DATI</font>
		<dhv:permission name="dpatAttivitaSalvaDefinitivo-view">
			...
			onclick="javascript : if (confirm('ATTENZIONE!!!Chiudere definitivamente l allegato 5? se clicchi OK non sarà MAI PIU possibile modificarlo.Se invece prevedi di apportare ancora modifiche clicca su ANNULLA')){window.location='Dpat.do?command=SalvaDefinitivo&id=<%=dpat.getId()%>&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>'}">
		</dhv:permission>
	</p>
...
<%} 
	...
	<script>
location.href="Dpat.do?command=DpatDetailGenerale&idAsl=<%=dpat.getIdAsl() %>&anno=<%=dpat.getAnno() %>";
</script>
<%} %>

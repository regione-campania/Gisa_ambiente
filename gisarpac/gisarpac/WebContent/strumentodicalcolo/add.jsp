...
function aggiornaDatiStruttura(idStruttura,edit,idArea)
{
	var area_sanita_animale = 1 ;
	percentualeValida = true;
	if (edit != 'view')
		{
	var fattori =document.getElementById("fattori_struttura_"+idStruttura).value;
	var percentuale = document.getElementById("percentuale_struttura_"+idStruttura).value;
	
	/**/
	if (checkNumber(percentuale)==true)
	{
		
		
		if(percentuale>15 && idArea!=area_sanita_animale)
			{
			percentualeValida = false;
			alert("Attenzione! La percentuale deve essere minore del 15 %");
			
			}
		else
			{
			if(percentuale>30 && idArea==area_sanita_animale)
				{
				percentualeValida = false;
				alert("Attenzione! La percentuale deve essere minore del 30 %");
				
				}
			}
		if(percentualeValida==true)
			{
		PopolaCombo.aggiornaDatiStruttura(idStruttura,fattori,percentuale,{callback:aggiornaDatiStrutturaCallback,async:false});
			}
	}
		}
	else
		{
			...
			alert('Foglio non Modificabile');
		}
	
	
}

function aggiornaDatiStrutturaCallback(beanStruttura)
{
	
	if (beanStruttura.idLookupAreaStruttura==1)
		document.getElementById("carico_effettivo_struttura_"+beanStruttura.codiceInternoFK).innerHTML = beanStruttura.caricoLavoroEffettivo +  " / "+beanStruttura.uba_ui ;
	else
		document.getElementById("carico_effettivo_struttura_"+beanStruttura.codiceInternoFK).innerHTML = beanStruttura.caricoLavoroEffettivo
		
		
		
		document.getElementById("uiAreaCorrente").innerHTML=beanStruttura.sommaUiArea;
}
...
<%-- 		<input type="button" value="Accedi al foglio Attivita'" onClick="window.location='Dpat.do?command=DpatDetailGenerale&idAsl=<%=DpatSDC.getIdAsl()%>&anno=<%=DpatSDC.getAnno()%>'"> --%>
		</dhv:permission>
...
<br><br>
			...
			<h1>LAVORAZIONE STRUMENTO DI CALCOLO PER LA STRUTTURA : <B><%=DpatSDC.getStrutturaAmbito().getDescrizioneAreaStruttureComplesse() %></B></h1>
			<%} %>
			<%
...
for (int i = 0 ; i < listStrutture.size(); i++)
{
	...
	%>
		...
		<td colspan="6"></td>
<%-- 		<td style="font-weight:bold;" align="center" rowspan="<%=(listaNominativiStruttura!= null && listaNominativiStruttura.size()>0)?""+((listaNominativiStruttura.size()+1)) :"2"%>"><%=struttura.getCaricoLavoroAnnuo() %></td> --%>
<%-- 		<td align="center" rowspan="<%=(listaNominativiStruttura!=null && listaNominativiStruttura.size()>0)?""+((listaNominativiStruttura.size()+1)) :"2"%>"><textarea <%=("view".equals(edit))? "readonly"  :""%>  rows="5" cols="32" style="font-weight:bold;" onchange="aggiornaDatiStruttura(<%=struttura.getCodiceInternoFK() %>,'<%=edit %>',<%=strutturaAmbito.getIdLookupAreaStruttura() %>)" id = "fattori_struttura_<%=struttura.getCodiceInternoFK() %>" name ="fattori_struttura" value = "<%=toHtml(struttura.getFattoriIncidentiSuCarico())%>"><%=toHtml( struttura.getFattoriIncidentiSuCarico()) %></textarea></td> --%>
		
<%-- 		<td align="center" rowspan="<%=(listaNominativiStruttura!=null && listaNominativiStruttura.size()>0)?""+((listaNominativiStruttura.size()+1)) :"2"%>"><input type = "text" style="font-weight:bold;text-align: center"  <%=("view".equals(edit))? "readonly"  :""%>  onchange="aggiornaDatiStruttura(<%=struttura.getCodiceInternoFK() %>,'<%=edit %>',<%=strutturaAmbito.getIdLookupAreaStruttura() %>)" id = "percentuale_struttura_<%=struttura.getCodiceInternoFK() %>" name = "percentuale_struttura" value = "<%=struttura.getPercentualeDaSottrarre()%>"></td> --%>
<%-- 		<td  style="font-weight:bold;" align="center" rowspan="<%=(listaNominativiStruttura!= null && listaNominativiStruttura.size()>0)?""+((listaNominativiStruttura.size()+1)) :"2"%>"><span id = "carico_effettivo_struttura_<%=struttura.getCodiceInternoFK()%>",'<%=edit %>'><%=struttura.getCaricoLavoroEffettivo() + ( (strutturaAmbito.getIdLookupAreaStruttura()==OiaNodo.AREA_A) ? " / "+struttura.getUba_ui():"") %></span></td> --%>
	</tr>
...
<%-- 					<td style="font-weight:bold;" align="center"><%=nominativo.getCaricoLavoroAnnuale() %></td> --%>
<%-- 					<td style="font-weight:bold;" align="center"><%=toHtml(nominativo.getFattoriIncidentiSuCarico()) %></td> --%>
<%-- 					<td style="font-weight:bold;"align="center"><%=(nominativo.getPercentualeDaSottrarre()>70 || nominativo.getSomma_parziale()>70) ?"<font color='black'>"+nominativo.getPercentualeDaSottrarre()+"</font>" :nominativo.getPercentualeDaSottrarre()+"" %></td> --%>
<%-- 					<td style="font-weight:bold;" align="center"><%=nominativo.getCaricoEffettivoAnnuale()  + ((strutturaAmbito.getIdLookupAreaStruttura()==OiaNodo.AREA_A) ? " / "+nominativo.getUba_ui() : "")%></td> --%>
...
function getStruttureComplesse(idAsl,anno,id,tipo,idPadre)
{
	...
	if (tipo=='12'){
		PopolaCombo.getStruttureComplesse(idAsl,anno,{callback: getStruttureComplesseCallBack,async:false});
...
<h1> <span id = "header"> Dati Struttura</span></h1>
       ...
       <table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
        ...
        %>
		         ...
		         <tr id = "scadenza">
					<td nowrap class="formLabel">DATA INIZIO VALIDITA</td>
					<td>

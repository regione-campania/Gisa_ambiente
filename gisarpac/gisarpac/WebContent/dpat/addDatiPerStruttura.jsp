...
<form action="Dpat.do?command=InsertDatiPerStruttura&idDpat=<%=dpat.getId()%>" method="post">
...
if (sezioni.size()>0){%>
	...
	<input type="button" id="annulla" value="annulla" onclick="window.location='Dpat.do?command=Lista'"/></p>
<%} else {%>
		Non esiste alcun DPAT (nell'anno corrente) specificato per l'asl selezionata.
		<p align="center">
		<input type="button" id="indietro" value="indietro" onclick="window.location='Dpat.do?command=Lista'"/></p>
<%} %>
</form>

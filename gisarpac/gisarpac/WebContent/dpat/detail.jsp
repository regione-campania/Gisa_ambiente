...
<table class="trails">
		<tr>
			<td><div align="left"><a href="Dpat.do">DPAT</a> &gt <a href="Dpat.do?command=Home&combo_area=<%=(dpat.getElencoStrutture()!= null && dpat.getElencoStrutture().size()>1) ? "-1" :dpat.getElencoStrutture().get(0).getId()  %>&idAsl=<%=dpat.getIdAsl()%>&anno=<%=dpat.getAnno()%>">Allegati DPAT</a> &gt <a href="DpatSDC.do?command=AddModify&idAsl=<%=dpat.getIdAsl()%>&combo_area=<%=dpat.getIdAreaSelezionata() %>&anno=<%=dpat.getAnno()%>">All. 5 Strumento di Calcolo <%=dpat.getAnno()%></a> &gt Attivita'  ASL :<%=ListaAsl.getSelectedValue(dpat.getIdAsl()) %></div></td>	
		</tr>
</table>
<!-- SERVER DOCUMENTALE -->
<p align="left">

...
DpatAttivita ind = (DpatAttivita)request.getAttribute("DettaglioAttivita");

ArrayList<DpatIndicatore> storia = ind.getStoriaAttivita();
%>
...
<div class="pager">
		...
		<select class="pagesize">
			...
			<option value="<%=storia.size()%>">Tutti</option>
		</select> / <%=storia.size()%>
	</div>
  	...
  	<table id ="tableRichiesteDaValidare" class="tableSorter">
		...
		for (DpatIndicatore indicatore : storia)
		{
			...
			<td><%=indicatore.getDescrizioneSezione() %> 	
			</td>
			<td><%=toHtml(indicatore.getAliasPiano()) %></td>
			<td><%=indicatore.getDescrizioneAttivita() %></td>
			<td><%=indicatore.getCodiceAlias() %></td>
			<td><%= (indicatore.getDataScadenza()!=null) ? toDateasString(indicatore.getDataScadenza()) : indicatore.getAnnoRiferimento()  %></td>

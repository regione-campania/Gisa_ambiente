...
<%@ include file="../initPage.jsp"%>
	<table class="trails" cellspacing="0">
		<tr>
			<td width="100%"><a href="Oia.do">Modellatore Organizzazione ASL</a> &gt <a href="Dpat.do?&idAsl=<%=idAsl%>&idPadre=<%=idPadre%>&anno=corrente">DPAT</a> &gt Elenco All.5
			</td>
		</tr>
	</table>
...
<%		for (int i=0;i<elencoDpat.size();i++){
			...
			<a href="Dpat.do?command=DpatDetailGenerale&idAsl=<%=d.getIdAsl()%>&idPadre=<%=idPadre%>&anno=<%=d.getAnno()%>&edit=<%=edit%>"><%=d.getAnno()%></a><br>
<% 		}
	} else {%>
		DPAT NON PRESENTE
<%  }%>
...
<%		for (int i=0;i<elencoDpatSC.size();i++){
	DpatStrumentoCalcolo d = (DpatStrumentoCalcolo)elencoDpatSC.get(i);%>
			<a href="DpatSDC.do?command=AddModify&idAsl=<%=d.getIdAsl()%>&idPadre=<%=idPadre%>&anno=<%=d.getAnno()%>&edit=view"><%=d.getAnno()%></a>
<% 		}
	} else {%>
		DPAT ALL. 2 STRUMENTI DI CALCOLO
<%  }%>

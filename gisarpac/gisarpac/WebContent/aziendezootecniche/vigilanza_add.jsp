...
<td>
  ...
  <a href="<%=NewTicket.getURlDettaglio() %>.do?command=Details&orgId=<%=OrgDetails.getAltId()()%>"><dhv:label name="">Scheda Anagrafica Operatore</dhv:label></a> >
  <a href="<%=NewTicket.getURlDettaglio() %>.do?command=ViewVigilanza&orgId=<%=OrgDetails.getAltId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
...
<%
	if(!popUp){
%>
<input type="submit" value="<dhv:label name="global.button.cancel">Cancel</dhv:label>" onClick="javascript:this.form.action='AziendeZootecniche.do?command=ViewVigilanza&orgId=<%=OrgDetails.getAltId()%>'">
<% } else { %>

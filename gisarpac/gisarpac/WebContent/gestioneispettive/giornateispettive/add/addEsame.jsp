...
<jsp:useBean id="ListaEsami" class="java.util.ArrayList" scope="request"/>
...
function checkForm(form){
	...
	var almenoUnEsame = false;

	var x = document.getElementsByName("esameId");
	...
	if (!almenoUnEsame){
		msg +="Selezionare almeno un esame.\n";
		esito = false;
	}
...
function backForm(form){
	form.action="GestioneGiornateIspettive.do?command=AddMotivo";
	loadModalWindow();
	form.submit();
}
...
function filtraRighe() {
	  ...
	  // Declare variables
	  var table = document.getElementById("tableEsami");
	  var input1 = document.getElementById("myInputEsame");
...
<form name="aggiungiCU" action="GestioneGiornateIspettive.do?command=AddTipoVerifica&auto-populate=true" onSubmit="" method="post">
...
<br/>
<table class="details" id ="tableEsami" name="tableEsami" cellpadding="10" cellspacing="10" width="100%" style="border-collapse: collapse">
<col width="5%">
<tr><th colspan="2"><center><b>ESAMI DELLA DOCUMENTAZIONE</b></center></th></tr>
...
<tr>
<th></th>
<th><input type="text" id="myInputEsame" onkeyup="filtraRighe()" placeholder="FILTRA ESAME" style="width: 100%"></th>
</tr>

<% 
if (ListaEsami.size()>0) {
for (int i = 0; i<ListaEsami.size(); i++) {
Esame esame = (Esame) ListaEsami.get(i); %>

<tr>
<td>
<input type="checkbox" id ="<%= esame.getCode()%>" name="esameId" value="<%= esame.getCode()%>"/>
<input type="hidden" readonly id ="esameNome_<%= esame.getCode()%>" name ="esameNome_<%= esame.getCode()%>" value="<%= esame.getEsame() %>"/>
</td>
<td><%= esame.getEsame() %></td>
</tr>
<% } %>

<%} else { %>
<tr><td colspan="2">PER QUESTA TECNICA NON SONO PREVISTI ESAMI. PROSEGUIRE.
<input type="hidden" checked readonly id ="-1" name ="esameId" value="-1"/>
</td></tr>
<%} %>
</table>	
...
JSONObject jsonObject = jsonEsami.getJSONObject(i);
%>

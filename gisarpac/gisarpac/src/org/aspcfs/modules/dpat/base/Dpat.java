...
public class Dpat extends GenericBean {
	...
	public void setObiettivo_in_ui(int obiettivo_in_ui) {
		this.obiettivo_in_ui = obiettivo_in_ui;
	}
	public int getCarico_in_ui() {
		return carico_in_ui;
	}
	public void setCarico_in_ui(int carico_in_cu) {
		this.carico_in_ui = carico_in_cu;
	}
	...
	public void builRecord(int id,Connection db,SystemStatus system,int idAreaSelezionata){
		try
		{
			...
			while (rs.next()){
				...
				this.setObiettivo_in_ui(rs.getInt("obiettivo_in_ui"));
				this.setCarico_in_ui(rs.getInt("carico_in_ui"));
...
//			}
			...
			String sql ="select * from (select distinct on (a.codice_interno_fk) a.codice_interno_fk as cifk  ,a.* from "
					...
					+ "case when anno=2016 then ui_struttura_foglio_att_iniziale else carico_lavoro_effettivo end as carico_lavoro_effettivo,"
					+"case when anno=2016 and o.id_lookup_area_struttura_asl=1 then (ui_struttura_foglio_att_iniziale * 48) else null end as uba_ui"
			...
			try {
				...
				while(rs.next()){
					...
					try
					{
						ds.setUbaUi(rs.getInt("uba_ui"));
						ds.setCaricoInUi(rs.getInt("carico_lavoro_effettivo"));
					}
					catch(Exception e)
					{
...
//		}
	...
	public boolean propagaConfiguratoreDpat(Connection db,int enteredby,int anno,ActionContext context ) throws SQLException
	{
		...
		try
		{
			...
			String verificaCoefficienti = "select  coefficiente from lookup_piano_monitoraggio_configuratore where enabled and coefficiente = -1 and id_padre>0";
			ResultSet rs = db.prepareStatement(verificaCoefficienti).executeQuery();
			if (rs.next())
				return false;
			pst=db.prepareStatement("INSERT INTO dpat_istanza(id,anno,entered,enteredby,stato) values(?,?,CURRENT_DATE,?,'TEMPORANEO')");
			
			int id = DatabaseUtils.getNextSeq(db,context, "dpat_istanza","id");
			pst.setInt(++i, id);
			pst.setInt(++i, anno);
			pst.setInt(++i, enteredby);
			pst.execute();
			
			String sql = "insert into dpat ( anno,id_asl,entered_by,modified,modified_by,entered,enabled,completo,id_dpat_istanza) values (?,?,291,current_date,291,current_date,true,false,?)";
			pst= db.prepareStatement(sql);
			pst.setInt(1, anno);
			pst.setInt(3, id);
			for(int asl = 201 ; asl<=207; asl++)
			{
				pst.setInt(2, asl);
				pst.execute();
			}
			...
			propagaSezioniConfiguratoreDpat(db,anno,id) ;
			propagaPianiConfiguratoreDpat(db,anno);
			propagaAttivitaConfiguratoreDpat(db,anno);
			propagaIndicatoriConfiguratoreDpat(db,anno);
			propagaCoefficientiConfiguratoreDpat(db,anno);
	...
	private boolean propagaCoefficientiConfiguratoreDpat(Connection db,int anno) throws SQLException
	{
		...
		try
		{
			...
			String insert = "insert into dpat_coefficiente (id_indicatore,coefficiente) "+
			"( "+
			"select id,coefficiente "+
			"from "+
			"( "+
			"select distinct ind.id,config_ind.coefficiente,config.id_sezione,config.ordinamento,config_ind.ordinamento_figli,config_ind.code as idpiano "+
			" from lookup_piano_monitoraggio_configuratore config "+
			"join lookup_piano_monitoraggio_configuratore config_ind on config_ind.id_padre=config.code "+
			"join dpat_sezione on config.id_sezione = dpat_sezione.code_lookup_sezioni_piani_monitoraggio "+
			"join dpat_piano p on p.id_sezione = dpat_sezione.id and config.code = p.code_lookup_piano_monitoraggio_padre "+
			"join dpat_attivita att on att.id_piano = p.id "+
			"join dpat_indicatore ind on ind.code_lookup_piano_monitoraggio_figli =config_ind.code "+
			" where config.id_padre<=0 and config.enabled and dpat_sezione.anno=? and config.enabled and p.enabled  "+
			" order by config.id_sezione,ordinamento "+
			")indicatori "+
			"); ";
